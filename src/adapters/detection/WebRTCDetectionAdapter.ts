import { DetectionPort, DetectionEvent } from '../../core/ports/DetectionPort';

type DetectionCallback = (event: DetectionEvent) => void;

export class WebRTCDetectionAdapter implements DetectionPort {
  ...
  start(pc: any, onDetection: DetectionCallback, soundThreshold = -50): void {
  ...
  private async checkAudioLevel(): Promise<void> {
    ...
    try {
      ...
      if (audioLevel > -100 && audioLevel > this.soundThreshold) {
        const now = Date.now();
